...
- `v2-r2` (后被 `v2-r3` 撤回) — Constraint ≡ Restraint scope-based unification；commit `d131a49`
- `v2-r3` — 撤回 `v2-r2` 合并：Constraint = 硬约束（未实现）、Restraint = 软惩罚；spec rename
- `v2-r4` — direction 3 通用扩展点模式（trait + N concrete pub structs；no Builtin* / wrapper / builder）
- `B.0` — 结构重构：删 `BuiltinConstraint` / `MoleculeConstraint` / `AtomConstraint` / `RegionConstraint` trait；建 `pub trait Restraint` + 14 具体 `*Restraint` struct 各持几何字段；`Vec<Arc<dyn Restraint>>` 内部存储；`Target::with_restraint` / `with_restraint_for_atoms` 公开 API；删 `pub use ... as Restraint` 别名；e2e mixture 498→499 ms +0.22%（p=0.92）远过软门禁 → fast-path follow-up 不需要
- `B.0b` — `pub trait Region` + `And`/`Or`/`Not` 组合子（解析梯度 chain rule）+ `RegionExt` 扩展 trait + `FromRegion<R>` blanket impl + 3 具体 Region（`InsideBoxRegion` / `InsideSphereRegion` / `OutsideSphereRegion`）；12 单元测试（boolean algebra / de Morgan / signed_distance signs / FromRegion gradient vs FD）；纯加性
- `B.2/B.3/B.5/B.6` — bundled commit：`InsideBoxRestraint::from_simbox(&SimBox)` + `Molpack::add_restraint(impl Restraint + 'static)`（per-target 广播，scope 等价律）+ `Handler::on_inner_iter` / `on_phase_end` 默认方法 + `PhaseReport` struct + 用户 plugin 类型平权测试；128 tests pass；e2e mixture 498→506 ms +0.93%（p=0.42）。**Phase B 完成**
- `B-cleanup` — 删 3 个 sentinel fns（`evaluate_unscaled_sentinel` / `run_iteration_sentinel` / `run_phase_sentinel`）+ 3 个 pin tests，per `molrs-perf` "delete F_sentinel after the next refactor cycle" 规则（Phase A→B 即 1 cycle）；4 个 bench 文件 trim 为 fn + caller permanent regression guards；CLAUDE.md trait 表 + Critical Conventions + Packing 子系统段落 rewrite；`molrs-pack/README.md` rewrite 为 user guide（Restraint + Region + Molpack::add_restraint + scope + 自定义 plugin）；`target.rs` 一处 stale `with_constraint()` doc comment 修；125 tests pass；clippy 0 pack warnings
