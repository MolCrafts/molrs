...
**Phase B — 公开新 trait，并行新旧 API**（v2-r3 修订：撤回 v2-r2 合并；15 内置是 Restraint 不是 Constraint）
- [ ] **B.0** Rename sweep：`BuiltinConstraint` → `BuiltinRestraint`；15 built-in 类型 `*Constraint` → `*Restraint`；`Target::with_constraint` → `with_restraint`、字段 `constraints` → `restraints`；文件 `src/constraint.rs` → `src/restraint.rs`、`src/constraints/` → `src/restraints/`、`tests/constraint.rs` → `tests/restraint.rs`；删除 `pub use BuiltinConstraint as Restraint` 别名；5 examples + README + doctests 一并更新。**不保留任何别名**（该 API 尚未向外发版，不承担向后兼容）。保留 `constrain_rot` / `rot_bound` / gencan 投影 bounds（唯一真正的硬约束语义）
- [ ] **B.1** 在 `molrs-pack` 公开 `pub trait Restraint: Send + Sync`（§6.2 签名：`f` / `fg` / `is_parallel_safe` / `name`）；`impl Restraint for BuiltinRestraint`（delegate 到既有 inherent 方法）；15 kind × trait-path vs inherent-path 数值等价单测。不公开 `Region` / `Selector`（§6.4 决议不需要独立 Selector trait；`Region` 推到 B.1b 如需）
- [ ] **B.2** opaque builder：`InsideBoxRestraint::new(...)` 等 builder 保持现有构造器签名但内部明确返回 `BuiltinRestraint`（不暴露 enum 内部表示）
- [ ] **B.3** `Molpack::add_restraint(impl Into<RestraintInput>)` — 接受 builder / `Box<dyn Restraint>` / Python；**实现 = 遍历 targets 各调一次 `with_restraint`**（global ≡ per-target × N，无独立存储路径）
...
- [ ] **B.6** 验收：用户能用纯 Rust 写自定义 `Restraint`（global 或 per-target 两种挂法都跑通）；基线无退化
...
| 现有代码中的对应 | `BuiltinRestraint{kind,params[9]}`（B.0 rename 后；pre-B.0 叫 `BuiltinConstraint`）+ `iratom_*` CSR | `Relaxer` / `RelaxerRunner`（A.3 rename，原名 `Hook`） | `handler.rs` 的 `Handler` |
| v2-r3 名字 | `Restraint`（**无别名**） | `Relaxer`（支持 MC/MD/梯度下降等多策略） | `Handler`（扩展） |
...
| 2 | `Restraint` | trait | 唯一的 penalty+gradient 抽象；global 或 per-target 均可挂；内置 + 用户 Rust + Python | `molrs-pack/src/restraint/` |
| 3 | `BuiltinRestraint` | struct | 15 种内置 soft penalty 的静态-dispatch 版本（kind:u8 + params[9]） | 私有 |
| 4 | `RestraintSet` | struct | 编译时分区（builtin / user-rust / user-python）+ CSR | 私有 |
...
pub trait Restraint: Send + Sync {
    ...
    /// Maps to existing `BuiltinRestraint::value(pos, scale, scale2)`.
    fn f(&self, x: &[F; 3], scale: F, scale2: F) -> F;

    /// Fused function + gradient. Hot path.
    /// Maps to existing `BuiltinRestraint::value` + `BuiltinRestraint::gradient` called
...
### 6.3 RestraintSet（静态 dispatch 内置 + dyn 扩展）

**这是 v2 最关键的性能保证点**。硬性要求：内置 15 种 restraint 绝不走 `Box<dyn>`。

```rust
// molrs-pack/src/restraint/set.rs

/// Tagged union mirroring current `BuiltinRestraint{kind:u8, params:[F;9]}`.
/// Do NOT box this; it lives inline in `RestraintSet::builtin_*` vectors.
#[derive(Clone, Copy)]
pub(crate) struct BuiltinRestraint {
    pub kind: u8,           // 2..=15, same numbering as Packmol
    pub params: [F; 9],
}
impl BuiltinRestraint {
    #[inline(always)]
...
/// Compiled restraint layout — partition done ONCE in Molpack::compile().
///
...
**禁止项（spec 级约束）**：
- ❌ `Vec<Box<dyn Restraint>>` 作为内置 restraint 的统一容器
- ❌ 热路径里做 `restraints.iter().partition(...)`
- ❌ 每原子一次 GIL acquire
- ✅ 编译时一次 partition；每 eval 一次 GIL acquire 包住全部 Python restraints
...
- 多原子累加契约：`fg` 签名保持单原子 `g: &mut [F;3]`——多原子展开由外层 `RestraintSet` 调度器调 `fg` 多次完成，避免 trait 签名特化到多原子形态。
...
let result = Molpack::new()
    ...
    .add_restraint(InsideBoxRestraint::from_simbox(&simbox))
    .add_restraint(OutsideSphereRestraint::new([0.0; 3], 5.0))
    .add_restraint(MyCustomRustRestraint::new(...))
    .add_restraint(py_restraint)
    // Region 组合自动转 Restraint
    .add_restraint(InsideSphereRestraint::new(c, r1).and(OutsideSphereRestraint::new(c, r2)))
    // 多原子 penalty 也走同一个 add_restraint；实例字段（k / weight / 是否进 frest）自决定
    .add_restraint(DistanceRestraint::new(atom_a, atom_b, target=1.5, k=10.0))
    ...
    .target(mol_b).with_restraint_for_atoms(&[0, 1, 2], LocalBiasRestraint::new(...))
...
│       · Move Builtin trait-impls back into BuiltinRestraint struct   │
