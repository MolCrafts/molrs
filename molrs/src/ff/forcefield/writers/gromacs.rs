...
fn format_params(
    ...
    for key in keys {
        ...
        let file_v = match (kind, *key) {
            ...
            // Canonical torsions store the phase in radians and barriers in
            // kcal/mol; the GROMACS file wants degrees and kJ/mol.
            (UnitKind::Dihedral, "phase" | "psi0") => v.to_degrees(),
            (UnitKind::Dihedral, "k" | "c0" | "c1" | "c2" | "c3" | "c4" | "c5") => v * KJ_PER_KCAL,
            // GROMACS buckingham writes `a b c6` with b = 1/rho in 1/nm.
            (UnitKind::Pair, "a") => v * KJ_PER_KCAL,
            (UnitKind::Pair, "rho") => {
                if v == 0.0 {
                    0.0
                } else {
            ...
            (UnitKind::Pair, "c" | "c6") => v * KJ_PER_KCAL / NM_TO_ANGSTROM.powi(6),
            (UnitKind::Pair, "c12") => v * KJ_PER_KCAL / NM_TO_ANGSTROM.powi(12),
            _ => v,
        };
