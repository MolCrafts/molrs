//! Configuration and periodic pair resolution for the soft packing potential.
//!
//! [`SoftSpec`] is the **builder** half of the soft potential: it holds the
//! topology and the parameters, resolves the non-bonded pairs for a given
//! configuration + box with molrs's own neighbour list, and hands back the pure
//! [`SoftPotential`] kernel that `ff` owns.
//!
//! It lives in `optimize` rather than beside the kernel because that is what it
//! parameterises — a minimizer's rebuild policy. `ff` owns potential *forms*;
//! deciding how often to re-resolve pairs as atoms move is
//! potential-driven minimization, and `optimize` may name `ff` while `ff` must
//! never name `optimize`.
//!
//! Construct the optimizer directly:
//! `SoftLbfgs::new(SoftSpec::from_frame(&frame), fmax, max_steps, max_step, memory)`.
...
impl SoftSpec {
    ...
    /// Resolve the pairs for `coords` (+ optional periodic cubic box) with
    /// molrs's neighbour list and return the pure [`SoftPotential`]. Non-bonded
    /// pairs use cutoff `max(rcut, sigma)`, exclude 1-2 / 1-3 neighbours, and
    /// carry the neighbour list's minimum-image shift; bond / 1-3 `r0`/`a0`
    /// (and shift) come from the minimum-image displacement in this
    /// configuration.
