...
| File | Origin | License |
|---|---|---|
| `molrs/data/oplsaa.xml` | OPLS-AA atom types & typing definitions from **foyer** | `MIT` (foyer) |
| `molrs/data/mmff94.xml`, `mmff94s.xml` (and workspace-root `data/` copies) | generated by `scripts/mmff_to_xml.py` from **RDKit** `Code/ForceField/MMFF/Params.cpp` (MMFF94/94s tables) | `BSD-3-Clause` (RDKit); MMFF94 parameters © Merck / T. A. Halgren, *J. Comput. Chem.* **17**, 490 (1996) |
