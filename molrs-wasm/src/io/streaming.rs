...
//! `WasmSdfStream`, `WasmDcdStream`, `WasmXtcStream`, `WasmTrrStream` —
//! so they are generated by the [`impl_wasm_traj_stream`] macro below.
//! Each only differs in *which* `FrameIndexBuilder` is constructed and
//! *which* `parse_frame_bytes` free function is invoked.
//!
...
macro_rules! impl_wasm_traj_stream {
    (
        ...
        impl $struct {
            ...
            /// Tell the indexer the source length when the host already
            /// knows it (DCD layout probe). No-op for self-describing
            /// formats.
            #[wasm_bindgen(js_name = hintTotalBytes)]
            pub fn hint_total_bytes(&mut self, total: f64) {
            ...
            pub fn parse_range_in_input(
                ...
                // DCD is the only format whose `parse_fn` consults decoder
                // context. Other formats ignore it so a stray
                // `setDecoderContext` cannot wrap XTC/TRR bytes in DCD magic
...
impl_wasm_traj_stream! {
    ...
    parse_fn    = parse_lammps_data_stream,
}
...
impl_wasm_traj_stream! {
    ...
    parse_fn    = parse_sdf_stream,
}

impl_wasm_traj_stream! {
    struct_name = WasmDcdStream,
    js_name     = WasmDcdStream,
    new_indexer = DcdIndexBuilder::new(),
    parse_fn    = parse_dcd_stream,
}

impl_wasm_traj_stream! {
    struct_name = WasmXtcStream,
    js_name     = WasmXtcStream,
    new_indexer = XtcIndexBuilder::new(),
    parse_fn    = parse_xtc_stream,
}

impl_wasm_traj_stream! {
    struct_name = WasmTrrStream,
    js_name     = WasmTrrStream,
    new_indexer = TrrIndexBuilder::new(),
    parse_fn    = parse_trr_stream,
}
