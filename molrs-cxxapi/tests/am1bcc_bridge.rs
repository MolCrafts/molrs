...
//! Atomiverse. The expected numbers are anchored on the antechamber oracle
//! (`molrs/tests/ff/typifier/antechamber_oracle.rs`, generated by AmberTools25),
...
/// The antechamber reference data, shared with the molrs-side typifier tests.
///
...
#[path = "../../molrs/tests/ff/typifier/antechamber_oracle.rs"]
mod antechamber_oracle;

use antechamber_oracle::{AntechamberCase, CASES};

/// antechamber writes charges to 4 decimals and the BCC increments carry 4, so
/// anything above 1e-4 is a real disagreement rather than formatting. Same
/// constant, same reason, as `molrs/tests/ff/typifier/am1bcc_antechamber.rs`.
...
fn case(name: &str) -> &'static AntechamberCase {
    CASES
        .iter()
        .find(|c| c.name == name)
        .unwrap_or_else(|| panic!("no antechamber oracle case named '{name}'"))
}
...
mod cxx {
    ...
    use super::{AntechamberCase, CHARGE_TOL, case};
    ...
    fn frame_of_case(case: &AntechamberCase) -> FrameRef {
    ...
    /// Compare against an antechamber column, atom by atom.
    fn assert_charges(what: &str, got: &[f64], want: &[f64]) {
        ...
        for (i, (got, want)) in got.iter().zip(want).enumerate() {
            assert!(
                ...
                "{what}: atom {i} is {got:.6} e, antechamber says {want:.6} e \
