...
def compute_gasteiger_charges(mol: Atomistic) -> List[Tuple[int, float]]:
    """Gasteiger/PEOE charges as ``(atom_handle, charge)`` pairs.

    A compat door onto :class:`GasteigerModel` — it delegates, so there is exactly
    one Gasteiger implementation and the two agree bitwise."""
    ...
...
class Perceive:
    ...
    ``find_aromaticity`` → ``is_aromatic`` (+ aromatic bond ``order`` 1.5, original
    under ``kekule_order``); ``find_hydrogens`` → adds H atoms and bonds;
