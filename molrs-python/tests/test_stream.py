...
@pytest.mark.skipif(
    not hasattr(molrs.stream, "FrameServer"),
    reason="FrameServer is native-only (absent on Pyodide)",
)
class TestFrameServer:
    def test_ephemeral_bind_reports_its_port(self):
        with molrs.stream.FrameServer("127.0.0.1:0") as server:
    ...
    def test_send_without_a_client_does_not_block(self):
        ...
        with molrs.stream.FrameServer("127.0.0.1:0", buffer_size=1) as server:
    ...
    def test_zero_buffer_size_raises(self):
        with pytest.raises(ValueError, match="buffer_size"):
            molrs.stream.FrameServer("127.0.0.1:0", buffer_size=0)

    def test_unknown_format_raises_before_binding(self):
        with pytest.raises(ValueError, match="unknown wire format"):
            molrs.stream.FrameServer("127.0.0.1:0", format="protobuf")

    def test_bad_address_raises_ioerror(self):
        with pytest.raises(OSError):
            molrs.stream.FrameServer("256.256.256.256:1")

    def test_use_after_close_raises(self):
        server = molrs.stream.FrameServer("127.0.0.1:0")
        server.close()
