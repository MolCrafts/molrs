...
Self-contained: fixtures are written with ``molrs.write_dcd``. No chemfiles
...
class TestReadDcd:
    def test_parses_written_trajectory(self, water_dcd):
        frames = molrs.read_dcd(str(water_dcd))
        assert len(frames) == 2
...
class TestDcdTrajReader:
    ...
    def test_iteration(self, water_dcd):
        reader = molrs.DCDTrajReader(str(water_dcd))
        n = len(reader)
...
class TestWriteDcd:
    def test_round_trip_atom_count(self, water_frame):
        with tempfile.NamedTemporaryFile(suffix=".dcd", delete=False) as tmp:
            tmpname = tmp.name
        try:
            molrs.write_dcd(tmpname, [water_frame, water_frame])
            frames = molrs.read_dcd(tmpname)
            assert len(frames) == 2
