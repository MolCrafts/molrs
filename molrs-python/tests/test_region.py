...
class TestSphere:
    def test_contains(self):
        s = molrs.Sphere(np.array([0.0, 0.0, 0.0], dtype=np.float32), 2.0)
        pts = np.array(
            [
                [0.0, 0.0, 0.0],  # center
                [1.0, 0.0, 0.0],  # inside
    ...
    def test_bounds(self):
        s = molrs.Sphere(np.array([1.0, 2.0, 3.0], dtype=np.float32), 2.0)
        b = s.bounds()
        assert b.shape == (3, 2)
    ...
    def test_bad_center_shape(self):
        with pytest.raises(ValueError, match="length 3"):
            molrs.Sphere(np.array([0.0, 0.0], dtype=np.float32), 1.0)

    def test_bad_points_shape(self):
        s = molrs.Sphere(np.array([0.0, 0.0, 0.0], dtype=np.float32), 1.0)
        with pytest.raises(ValueError, match="N, 3"):
            s.contains(np.ones((3, 2), dtype=np.float32))

    def test_repr(self):
        s = molrs.Sphere(np.array([1.0, 2.0, 3.0], dtype=np.float32), 5.0)
        r = repr(s)
        assert "Sphere" in r
        assert "5.00" in r
...
class TestHollowSphere:
    def test_contains(self):
        hs = molrs.HollowSphere(
            np.array([0.0, 0.0, 0.0], dtype=np.float32), 2.0, 5.0
        )
        pts = np.array(
            [
...
class TestRegionAnd:
    def test_sphere_and_sphere(self):
        s1 = molrs.Sphere(np.array([0.0, 0.0, 0.0], dtype=np.float32), 3.0)
        s2 = molrs.Sphere(np.array([2.0, 0.0, 0.0], dtype=np.float32), 3.0)
        intersection = s1 & s2
    ...
    def test_hollow_and_sphere(self):
        hs = molrs.HollowSphere(
            np.array([0.0, 0.0, 0.0], dtype=np.float32), 1.0, 5.0
        )
        s = molrs.Sphere(np.array([3.0, 0.0, 0.0], dtype=np.float32), 3.0)
        result = hs & s
...
class TestRegionOr:
    def test_sphere_or_sphere(self):
        s1 = molrs.Sphere(np.array([0.0, 0.0, 0.0], dtype=np.float32), 1.0)
        s2 = molrs.Sphere(np.array([5.0, 0.0, 0.0], dtype=np.float32), 1.0)
        union = s1 | s2
...
class TestRegionChaining:
    def test_shell_via_and_not(self):
        outer = molrs.Sphere(np.array([0.0, 0.0, 0.0], dtype=np.float32), 5.0)
        inner = molrs.Sphere(np.array([0.0, 0.0, 0.0], dtype=np.float32), 2.0)
        shell = outer & (~inner)
    ...
    def test_region_and_region(self):
        s1 = molrs.Sphere(np.array([0.0, 0.0, 0.0], dtype=np.float32), 5.0)
        s2 = molrs.Sphere(np.array([0.0, 0.0, 0.0], dtype=np.float32), 3.0)
        r1 = s1 & s2
        r2 = ~s2
